...
export default function Base64Encoder() {
  ...
  const handleProcess = (text: string, currentMode: 'encode' | 'decode') => {
    ...
    try {
      ...
      setError(currentMode === 'decode' ? 'Invalid Base64 string' : 'Error encoding string');
      setOutput('');
    }
  };
  ...
  const toggleMode = () => {
    ...
    trackToolAction('base64-encode-decode', `switch_to_${newMode}`);
  };

  const handleCopy = () => {
    navigator.clipboard.writeText(output);
    trackToolAction('base64-encode-decode', 'copy_output');
  };
